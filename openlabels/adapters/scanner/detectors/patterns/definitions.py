...
add_pattern(r'(?:phone|tel|fax|call|contact)[:\s]+([()\d\s+.-]{10,20})', 'PHONE', CONFIDENCE_RELIABLE, 1, re.I)
...
add_pattern(r'(?:phone|tel|call|contact)[:\s]+\(([S5]\d{2})\)\s*(\d{3})[-.]?(\d{4})', 'PHONE', CONFIDENCE_MEDIUM_LOW, 0, re.I)
add_pattern(r'(?:phone|tel|call|contact)[:\s]+\((\d[S5]\d)\)\s*(\d{3})[-.]?(\d{4})', 'PHONE', CONFIDENCE_MEDIUM_LOW, 0, re.I)
add_pattern(r'(?:phone|tel|call|contact)[:\s]+\((\d{2}[S5])\)\s*(\d{3})[-.]?(\d{4})', 'PHONE', CONFIDENCE_MEDIUM_LOW, 0, re.I)
# Phone with l/I for 1: "(555) l23-4567"
add_pattern(r'(?:phone|tel|call|contact)[:\s]+\((\d{3})\)\s*([lI1]\d{2})[-.]?(\d{4})', 'PHONE', CONFIDENCE_MEDIUM_LOW, 0, re.I)
...
add_pattern(r'(?:DOB|Date)[:\s]+(\d{1,2}\.\d{1,2}\.\d{4})', 'DATE', CONFIDENCE_LOW, 1, re.I)
add_pattern(r'\b(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2},?\s+\d{4}\b', 'DATE', CONFIDENCE_MINIMAL, 0, re.I)
add_pattern(r'\b\d{1,2}\s+(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{4}\b', 'DATE', CONFIDENCE_MINIMAL, 0, re.I)
# Edge case: "November 3., 1986" - day with period before comma/year (evasion pattern)
add_pattern(r'\b(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2}\.,\s*\d{4}\b', 'DATE', CONFIDENCE_BORDERLINE, 0, re.I)
# Abbreviated month names: "Oct 11, 1984", "Mar 19, 1988", "Jan 15th, 1980"
add_pattern(r'\b(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)\.?\s+\d{1,2}(?:st|nd|rd|th)?,?\s+\d{4}\b', 'DATE', CONFIDENCE_MINIMAL, 0, re.I)
add_pattern(r'\b\d{1,2}\s+(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)\.?\s+\d{4}\b', 'DATE', CONFIDENCE_MINIMAL, 0, re.I)
# DOB with abbreviated months
add_pattern(r'(?:DOB|Date\s+of\s+Birth|Birth\s*date)[:\s]+((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)\.?\s+\d{1,2},?\s+\d{4})', 'DATE_DOB', CONFIDENCE_HIGH, 1, re.I)
add_pattern(r'(?:DOB|Date\s+of\s+Birth|Birth\s*date)[:\s]+(\d{1,2}[/\-]\d{1,2}[/\-]\d{2,4})', 'DATE_DOB', CONFIDENCE_HIGH, 1, re.I)
add_pattern(r'(?:admission|admit|discharge)[:\s]+(\d{1,2}[/\-]\d{1,2}[/\-]\d{2,4})', 'DATE', CONFIDENCE_MEDIUM, 1, re.I)

# === Ordinal Date Formats ===
# "3rd of March, 1990", "1st of January, 2020"
add_pattern(r'\b(\d{1,2}(?:st|nd|rd|th)\s+of\s+(?:January|February|March|April|May|June|July|August|September|October|November|December)(?:\s*,?\s*\d{4})?)\b', 'DATE', CONFIDENCE_WEAK, 0, re.I)
# "3rd of March" (without year), "22nd of December"
add_pattern(r'\b(\d{1,2}(?:st|nd|rd|th)\s+of\s+(?:January|February|March|April|May|June|July|August|September|October|November|December))\b', 'DATE', CONFIDENCE_MINIMAL, 0, re.I)
# "3rd March 1990", "1st January 2020" (ordinal without "of")
add_pattern(r'\b(\d{1,2}(?:st|nd|rd|th)\s+(?:January|February|March|April|May|June|July|August|September|October|November|December)(?:\s*,?\s*\d{4})?)\b', 'DATE', CONFIDENCE_BORDERLINE, 0, re.I)
# "the 15th of January" (with "the")
add_pattern(r'\b(the\s+\d{1,2}(?:st|nd|rd|th)\s+of\s+(?:January|February|March|April|May|June|July|August|September|October|November|December))\b', 'DATE', CONFIDENCE_WEAK, 0, re.I)

# === Weekday + Date Formats ===
# "Fri, Mar 3, 2024", "Monday, January 15, 2024"
add_pattern(r'\b((?:Mon|Tue|Wed|Thu|Fri|Sat|Sun|Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday)\s*,?\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec|January|February|March|April|May|June|July|August|September|October|November|December)\.?\s+\d{1,2}\s*,?\s*\d{4})\b', 'DATE', CONFIDENCE_MARGINAL, 0, re.I)

# === Date ranges with written months ===
# "between January 1 and January 15"
add_pattern(r'\b((?:between|from)\s+(?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2})\b', 'DATE', CONFIDENCE_MINIMAL, 0, re.I)
add_pattern(r'\b((?:and|to|through)\s+(?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2})\b', 'DATE', CONFIDENCE_MINIMAL, 0, re.I)
...
add_pattern(r'\b(\d{1,2}:\d{2}\s*(?:AM|PM|am|pm|a\.m\.|p\.m\.))\b', 'TIME', CONFIDENCE_MEDIUM_LOW, 0, re.I)
# With seconds: "11:30:45 PM"
add_pattern(r'\b(\d{1,2}:\d{2}:\d{2}\s*(?:AM|PM|am|pm|a\.m\.|p\.m\.))\b', 'TIME', CONFIDENCE_MEDIUM_LOW, 0, re.I)
# Contextual: "at 3:30 PM", "@ 11:45"
add_pattern(r'(?:at|@)\s*(\d{1,2}:\d{2}\s*(?:AM|PM|am|pm)?)\b', 'TIME', CONFIDENCE_LOW, 1, re.I)
# Labeled: "Time: 14:30", "recorded at 2:15 PM"
add_pattern(r'(?:time|recorded|documented|signed)[:\s]+(\d{1,2}:\d{2}(?::\d{2})?\s*(?:AM|PM|am|pm)?)', 'TIME', CONFIDENCE_MEDIUM, 1, re.I)
...
add_pattern(r'\b(\d{1,3})\s*y/?o\b', 'AGE', CONFIDENCE_MEDIUM_LOW, 1, re.I)
...
# "Bed: 2", "Bed 3A"
add_pattern(r'(?:Bed)[:\s#]+(\d{1,2}[A-Z]?)', 'ROOM', CONFIDENCE_LOW, 1, re.I)
...
add_pattern(rf'((?:Dr\.?|Doctor)[ \t]+{_INITIAL}[ \t]+{_NAME})', 'NAME_PROVIDER', CONFIDENCE_MEDIUM, 1, re.I)
add_pattern(rf'((?:Dr\.?|Doctor)[ \t]+{_DOUBLE_INITIAL}[ \t]+{_NAME})', 'NAME_PROVIDER', CONFIDENCE_MEDIUM, 1, re.I)
...
add_pattern(rf'(?:Electronically\s+signed|E-signed|Authenticated|Verified|Approved)\s+(?:by)[:\s]+({_NAME}(?:[ \t]+{_NAME}){{0,2}},?\s*{_CREDENTIALS})', 'NAME_PROVIDER', CONFIDENCE_NEAR_CERTAIN, 1, re.I)
add_pattern(rf'(?:Electronically\s+signed|E-signed|Authenticated|Verified|Approved)\s+(?:by)[:\s]+((?:Dr\.?|Doctor)[ \t]+{_NAME}(?:[ \t]+{_NAME}){{0,2}})', 'NAME_PROVIDER', CONFIDENCE_NEAR_CERTAIN, 1, re.I)

# Lab/clinical context: "drawn by J. Wilson" "reviewed by A. Smith MD"
add_pattern(rf'(?:drawn|reviewed|verified|reported|signed|approved|dictated|transcribed|entered|ordered)\s+(?:by|per)[:\s]+({_NAME}(?:[ \t]+{_NAME}){{0,2}},?\s*{_CREDENTIALS})', 'NAME_PROVIDER', CONFIDENCE_MEDIUM_LOW, 1, re.I)
...
add_pattern(rf'(?:cc|CC)[:\s]+((?:Dr\.?|Doctor)[ \t]+{_NAME}(?:[ \t]+{_NAME}){{0,2}})', 'NAME_PROVIDER', CONFIDENCE_LOW, 1, re.I)
add_pattern(rf'(?:cc|CC)[:\s]+({_NAME}(?:[ \t]+{_NAME}){{0,2}},?\s*{_CREDENTIALS})', 'NAME_PROVIDER', CONFIDENCE_LOW, 1, re.I)
...
add_pattern(rf'\b(?:Nurse|NP|PA|RN):\s*({_NAME}(?:[ \t]+{_NAME}){{0,2}})', 'NAME_PROVIDER', CONFIDENCE_LOW, 1, re.I)
...
add_pattern(rf'(?:Provider|Attending|Referring|Ordering|Treating|Primary\s+Care|Consultant)[:\s]+({_NAME}(?:[ \t]+{_NAME}){{1,2}})', 'NAME_PROVIDER', CONFIDENCE_HIGH_MEDIUM, 1, re.I)
add_pattern(rf'(?:Provider|Attending|Referring|Ordering|Treating|Primary\s+Care|Consultant)[:\s]+((?:Dr\.?|Doctor)[ \t]+{_NAME}(?:[ \t]+{_NAME}){{0,2}})', 'NAME_PROVIDER', CONFIDENCE_HIGH, 1, re.I)
...
# Signature patterns
add_pattern(rf'(?:Provider\s+Signature)[:\s]*({_NAME}(?:[ \t]+{_NAME}){{1,2}})', 'NAME_PROVIDER', CONFIDENCE_HIGH_MEDIUM, 1, re.I)
...
add_pattern(rf'(?:School\s+)?(?:Counselor|Social\s*Worker|Psychologist|Principal|Teacher)[:\s]+({_NAME}(?:[ \t]+{_NAME}){{1,2}},?\s*{_CREDENTIALS})', 'NAME', CONFIDENCE_HIGH_MEDIUM, 1, re.I)
add_pattern(rf'(?:School\s+)?(?:Counselor|Social\s*Worker|Psychologist|Principal|Teacher)[:\s]+({_NAME}[ \t]+{_MIDDLE_INITIAL}[ \t]+{_NAME})', 'NAME', CONFIDENCE_RELIABLE, 1, re.I)
add_pattern(rf'(?:School\s+)?(?:Counselor|Social\s*Worker|Psychologist|Principal|Teacher)[:\s]+({_NAME}[ \t]+{_MIDDLE_INITIAL}[ \t]+{_NAME},?\s*{_CREDENTIALS})', 'NAME', CONFIDENCE_HIGH_MEDIUM, 1, re.I)
...
add_pattern(r'(?:RESTR|RESTRICTION)[:\s]*(?:NONE|[A-Z])\s+([A-Z][a-z]+\s+[A-Z][a-z]+)(?=\s+\d|\s*$)', 'NAME', CONFIDENCE_LOW, 1, re.I)
...
add_pattern(r'(?:DOB)[:\s]+\d{1,2}[/-]\d{1,2}[/-]\d{2,4}\s+([A-Z]{2,20})(?=\s+\d|\s*$)', 'NAME', CONFIDENCE_MARGINAL, 1, re.I)
...
add_pattern(rf'(?:Nom|Prénom|Nom\s+de\s+famille)[:\s]+({_NAME}(?:[ \t]+{_NAME}){{0,2}})', 'NAME', CONFIDENCE_MEDIUM_LOW, 1, re.I)
# German: Name, Vorname, Nachname (name, first name, last name)
add_pattern(rf'(?:Vorname|Nachname|Familienname)[:\s]+({_NAME}(?:[ \t]+{_NAME}){{0,2}})', 'NAME', CONFIDENCE_MEDIUM_LOW, 1, re.I)
# Spanish: Nombre, Apellido (name, surname)
add_pattern(rf'(?:Nombre|Apellido|Apellidos)[:\s]+({_NAME}(?:[ \t]+{_NAME}){{0,2}})', 'NAME', CONFIDENCE_MEDIUM_LOW, 1, re.I)
# Italian: Nome, Cognome (name, surname)
add_pattern(rf'(?:Nome|Cognome)[:\s]+({_NAME}(?:[ \t]+{_NAME}){{0,2}})', 'NAME', CONFIDENCE_MEDIUM_LOW, 1, re.I)
# Dutch: Naam, Voornaam, Achternaam (name, first name, last name)
add_pattern(rf'(?:Naam|Voornaam|Achternaam)[:\s]+({_NAME}(?:[ \t]+{_NAME}){{0,2}})', 'NAME', CONFIDENCE_MEDIUM_LOW, 1, re.I)
# Portuguese: Nome, Sobrenome (name, surname)
add_pattern(rf'(?:Sobrenome)[:\s]+({_NAME}(?:[ \t]+{_NAME}){{0,2}})', 'NAME', CONFIDENCE_MEDIUM_LOW, 1, re.I)
# Full name field (international): "Full Name:", "Complete Name:"
add_pattern(rf'(?:Full\s+Name|Complete\s+Name|Legal\s+Name|Vollständiger\s+Name|Nom\s+complet|Nombre\s+completo)[:\s]+({_NAME}(?:[ \t]+{_NAME}){{1,3}})', 'NAME', CONFIDENCE_MEDIUM, 1, re.I)

# === PATIENT NAME PATTERNS ===

# Patient labeled patterns - REQUIRE COLON to avoid matching "Patient reports..."
add_pattern(rf'(?:Patient(?:\s+Name)?|Pt):\s*({_NAME}(?:[ \t]+{_NAME}){{1,3}})', 'NAME_PATIENT', CONFIDENCE_RELIABLE, 1, re.I)
...
add_pattern(rf'(?:Name):\s*({_NAME}(?:[ \t]+{_NAME}){{1,3}})', 'NAME_PATIENT', CONFIDENCE_MEDIUM_LOW, 1, re.I)
add_pattern(rf'(?:RE|Re|Regarding):\s*({_NAME}(?:[ \t]+{_NAME}){{1,3}})\s*\(', 'NAME_PATIENT', CONFIDENCE_MEDIUM, 1, re.I)
# Last, First format common in referrals: "RE: Smith, John" - capture as "Smith, John"
add_pattern(rf'(?:RE|Re|Regarding):\s*({_NAME},\s*{_NAME}(?:[ \t]+{_NAME}){{0,1}})', 'NAME_PATIENT', CONFIDENCE_MEDIUM, 1, re.I)

# Single labeled name: "Patient: John" - requires explicit colon
add_pattern(rf'(?:Patient):\s*({_NAME})\b', 'NAME_PATIENT', CONFIDENCE_MINIMAL, 1, re.I)

# Patient names with initials: "Patient: A. Whitaker", "Patient: A. B. Smith"
add_pattern(rf'(?:Patient(?:\s+Name)?|Pt):\s*({_INITIAL}[ \t]+{_NAME})', 'NAME_PATIENT', CONFIDENCE_MEDIUM, 1, re.I)
add_pattern(rf'(?:Patient(?:\s+Name)?|Pt):\s*({_DOUBLE_INITIAL}[ \t]+{_NAME})', 'NAME_PATIENT', CONFIDENCE_MEDIUM, 1, re.I)
# Patient names with middle initial: "Patient: John A. Smith"
add_pattern(rf'(?:Patient(?:\s+Name)?|Pt):\s*({_NAME}[ \t]+{_INITIAL}[ \t]+{_NAME})', 'NAME_PATIENT', CONFIDENCE_RELIABLE, 1, re.I)
...
add_pattern(rf'(?:prescribed|ordered|given|administered|dispensed)\s+(?:to|for)\s+({_NAME},\s+{_NAME})', 'NAME_PATIENT', CONFIDENCE_MINIMAL, 1, re.I)

# Inline names: "the patient, John Smith, arrived" - comma-delimited name
add_pattern(rf'(?:(?:the)\s+)?(?:patient),\s+({_NAME}(?:[ \t]+{_NAME}){{1,2}}),', 'NAME_PATIENT', CONFIDENCE_BORDERLINE, 1, re.I)
...
add_pattern(rf'\b(?:Mr\.?|Mrs\.?|Ms\.?|Miss)[ \t]+({_NAME}(?:[ \t]+{_NAME}){{0,2}})', 'NAME_PATIENT', CONFIDENCE_MEDIUM, 1, re.I)
...
add_pattern(rf'\b(?:Mr\.?|Mrs\.?|Ms\.?|Miss)[ \t]+({_INITIAL}[ \t]+{_NAME})', 'NAME_PATIENT', CONFIDENCE_MEDIUM, 1, re.I)
add_pattern(rf'\b(?:Mr\.?|Mrs\.?|Ms\.?|Miss)[ \t]+({_DOUBLE_INITIAL}[ \t]+{_NAME})', 'NAME_PATIENT', CONFIDENCE_MEDIUM, 1, re.I)
# With middle initial: "Mr. John A. Smith"
add_pattern(rf'\b(?:Mr\.?|Mrs\.?|Ms\.?|Miss)[ \t]+({_NAME}[ \t]+{_INITIAL}[ \t]+{_NAME})', 'NAME_PATIENT', CONFIDENCE_RELIABLE, 1, re.I)

# === RELATIVE/FAMILY NAME PATTERNS ===

# Explicit labels
add_pattern(rf'(?:Emergency\s+Contact|Next\s+of\s+Kin|NOK)[:\s]+({_NAME}(?:[ \t]+{_NAME}){{0,3}})', 'NAME_RELATIVE', CONFIDENCE_MEDIUM_LOW, 1, re.I)
add_pattern(rf'(?:Spouse|Partner|Guardian|Caregiver)[:\s]+({_NAME}(?:[ \t]+{_NAME}){{0,2}})', 'NAME_RELATIVE', CONFIDENCE_MEDIUM_LOW, 1, re.I)
...
add_pattern(rf'\b(?:husband|wife|spouse|partner|son|daughter|mother|father|brother|sister|parent|child|guardian)[ \t]+({_NAME}(?:[ \t]+{_NAME}){{0,2}})', 'NAME_RELATIVE', CONFIDENCE_MARGINAL, 1, re.I)
# Possessive: "patient's husband John", "her mother Mary"
add_pattern(rf'\b(?:patient\'?s?|his|her|their)[ \t]+(?:husband|wife|spouse|partner|son|daughter|mother|father|brother|sister|parent|child)[ \t]+({_NAME}(?:[ \t]+{_NAME}){{0,2}})', 'NAME_RELATIVE', CONFIDENCE_LOW, 1, re.I)
# "mother's name is Sarah", "father is John Smith"
add_pattern(rf'\b(?:mother|father|spouse|partner|guardian)(?:\'s[ \t]+name)?[ \t]+(?:is|was)[ \t]+({_NAME}(?:[ \t]+{_NAME}){{0,2}})', 'NAME_RELATIVE', CONFIDENCE_WEAK, 1, re.I)
...
add_pattern(rf'\b(?:my\s+name\s+is|I\s+am|I\'m)[ \t]+({_NAME}(?:[ \t]+{_NAME}){{0,2}})', 'NAME_PATIENT', CONFIDENCE_MEDIUM, 1, re.I)
# "this is John Smith" (phone/intro context)
add_pattern(rf'\bthis\s+is[ \t]+({_NAME}(?:[ \t]+{_NAME}){{0,2}})(?:\s+speaking|\s+calling)?', 'NAME_PATIENT', CONFIDENCE_LOW, 1, re.I)
...
add_pattern(r'(?:patient\s+ID|patient\s*#|pt\s+ID)[:\s#]+([A-Z]*-?\d{6,12}[A-Z]*)', 'MRN', CONFIDENCE_MEDIUM_LOW, 1, re.I)  # "patient ID" variant
add_pattern(r'(?:Encounter|Visit)[:\s#]+([A-Z]*\d{6,12}[A-Z]*)', 'ENCOUNTER_ID', CONFIDENCE_MEDIUM, 1, re.I)
...
add_pattern(r'(?:NPI)[:\s#]+(\d{10})\b', 'NPI', CONFIDENCE_HIGH, 1, re.I)
# Contextual: "provider NPI 1234567890"
add_pattern(r'(?:provider|physician|prescriber|ordering)\s+NPI[:\s#]*(\d{10})\b', 'NPI', CONFIDENCE_RELIABLE, 1, re.I)
...
add_pattern(rf'(?:Beneficiary\s*ID)[:\s#]*({_MBI_PATTERN})', 'MEDICARE_ID', CONFIDENCE_HIGH, 1, re.I)
# After other Medicare labels like "Medicare ID (MBI):"
add_pattern(rf'(?:ID\s*\(MBI\))[:\s#]*({_MBI_PATTERN})', 'MEDICARE_ID', CONFIDENCE_NEAR_CERTAIN, 1, re.I)
...
add_pattern(r'(?:RXBIN|RX\s*BIN)[:\s]+(\d{6})', 'PHARMACY_ID', CONFIDENCE_MEDIUM, 1, re.I)
add_pattern(r'(?:RXPCN|RX\s*PCN)[:\s]+([A-Z0-9]{4,10})', 'PHARMACY_ID', CONFIDENCE_MEDIUM_LOW, 1, re.I)
add_pattern(r'(?:Group(?:\s*(?:Number|No|#))?)[:\s#]+([A-Z0-9-]{4,15})', 'HEALTH_PLAN_ID', CONFIDENCE_MINIMAL, 1, re.I)

# Member ID with letter prefix and hyphen (e.g., BC-993812, BVH-882391)
add_pattern(r'(?:Member\s*ID)[:\s#]+([A-Z]{2,4}-\d{5,12})', 'MEMBER_ID', CONFIDENCE_RELIABLE, 1, re.I)
...
add_pattern(rf'(?:{_PAYER_PREFIXES})[- ]?([A-Z]*\d[A-Z0-9]{{5,14}})', 'HEALTH_PLAN_ID', CONFIDENCE_MEDIUM, 1, re.I)
...
add_pattern(rf'({_FACILITY_PREFIX}\s+(?:{_MEDICAL_SPECIALTY})\s+(?:Clinic|Center|Associates|Practice|Group|Specialists))\b', 'FACILITY', CONFIDENCE_MEDIUM, 1, re.I)

# Multi-part specialty facilities with "&": "Pulmonary & Sleep Center", "Cardiology & Vascular Associates"
add_pattern(rf'((?:{_MEDICAL_SPECIALTY})\s+(?:&|and)\s+(?:{_MEDICAL_SPECIALTY})\s+(?:Center|Clinic|Associates|Institute|Specialists))\b', 'FACILITY', CONFIDENCE_RELIABLE, 1, re.I)

# "[Name] Pulmonary & Sleep Center" (name prefix + specialty combo)
add_pattern(rf'({_FACILITY_PREFIX}\s+(?:{_MEDICAL_SPECIALTY})\s+(?:&|and)\s+(?:{_MEDICAL_SPECIALTY})\s+(?:Center|Clinic|Associates))\b', 'FACILITY', CONFIDENCE_RELIABLE, 1, re.I)

# Context-labeled facilities: "Clinic:", "Hospital:", "Center:" followed by name
add_pattern(rf'(?:Clinic|Hospital|Center|Practice)[:\s]+({_FACILITY_PREFIX}(?:\s+(?:{_MEDICAL_SPECIALTY}))?(?:\s+(?:&|and)\s+[A-Z][a-z]+)*(?:\s+(?:Center|Clinic|Associates|Practice))?)', 'FACILITY', CONFIDENCE_MEDIUM, 1, re.I)

# Standalone specialty practice names: "Pulmonary Associates", "Sleep Center", "Pain Specialists"
add_pattern(rf'\b((?:{_MEDICAL_SPECIALTY})\s+(?:Associates|Specialists|Center|Clinic|Practice|Group|Partners))\b', 'FACILITY', CONFIDENCE_LOW, 1, re.I)
...
add_pattern(rf'((?:{_PHARMACY_CHAINS})(?:\s+Pharmacy)?(?:\s*#?\d{{3,6}})?)', 'FACILITY', CONFIDENCE_RELIABLE, 1, re.I)
# "Preferred Pharmacy:" or "Pharmacy:" label followed by pharmacy name
add_pattern(rf'(?:Preferred\s+)?Pharmacy[:\s]+((?:{_PHARMACY_CHAINS})(?:\s+Pharmacy)?(?:\s*#?\d{{3,6}})?)', 'FACILITY', CONFIDENCE_HIGH_MEDIUM, 1, re.I)
...
# === IMEI ===
add_pattern(r'(?:IMEI)[:\s]+(\d{15})', 'IMEI', CONFIDENCE_HIGH, 1, re.I)
...
add_pattern(r'(?:Device\s*(?:ID|Identifier|Serial))[:\s]+([A-Z0-9]{6,20})', 'DEVICE_ID', CONFIDENCE_RELIABLE, 1, re.I)
...
add_pattern(r'(?:nom d\'utilisateur|benutzername|usuario|gebruikersnaam|nome utente|usuário|utilisateur)[:\s]+([\w._-]{3,30})', 'USERNAME', CONFIDENCE_LOW, 1, re.I | re.UNICODE)
...
add_pattern(r'(?:kennwort|passwort|mot\s+de\s+passe|contraseña|wachtwoord|senha|parola\s+d\'ordine)[:\s]+([^\s]{4,50})', 'PASSWORD', CONFIDENCE_MEDIUM, 1, re.I | re.UNICODE)
# Authentication context: "credentials: password", "secret: xxxxx"
add_pattern(r'(?:credential|secret|auth\s+key|api\s+key|access\s+key|secret\s+key)[:\s]+([^\s]{8,100})', 'PASSWORD', CONFIDENCE_MEDIUM_LOW, 1, re.I)
# Temp/initial password context
add_pattern(r'(?:temporary|temp|initial|default)\s+(?:password|pwd|passcode)[:\s]+([^\s]{4,50})', 'PASSWORD', CONFIDENCE_RELIABLE, 1, re.I)
...
add_pattern(r'(?:DL|License)[:\s]+(\d{9})\b', 'DRIVER_LICENSE', CONFIDENCE_LOW, 1, re.I)

# --- Pennsylvania: 8 digits ---
add_pattern(r'\b(\d{8})\b(?=.*(?:PA|Pennsylvania|DL|License))', 'DRIVER_LICENSE', CONFIDENCE_MINIMAL, 1, re.I)
...
add_pattern(r'(?:CO|Colorado|DL)[:\s]+(\d{9})\b', 'DRIVER_LICENSE', CONFIDENCE_WEAK, 1, re.I)

# --- Nevada: 9-12 digits, often starts with X or 9 ---
add_pattern(r'\b(X\d{8,11})\b', 'DRIVER_LICENSE', CONFIDENCE_LOW, 1)
add_pattern(r'(?:NV|Nevada|DL)[:\s]+(\d{9,12})\b', 'DRIVER_LICENSE', CONFIDENCE_BORDERLINE, 1, re.I)
...
add_pattern(r'(?:AZ|Arizona|DL)[:\s]+([A-Z]?\d{8,9})\b', 'DRIVER_LICENSE', CONFIDENCE_WEAK, 1, re.I)
...
add_pattern(r'(?:IN|Indiana|DL)[:\s]+(\d{10})\b', 'DRIVER_LICENSE', CONFIDENCE_BORDERLINE, 1, re.I)
...
add_pattern(r'(?:CT|Connecticut|DL)[:\s]+(\d{9})\b', 'DRIVER_LICENSE', CONFIDENCE_BORDERLINE, 1, re.I)

# --- Texas: 8 digits (with context) ---
add_pattern(r'(?:TX|Texas|DL)[:\s]+(\d{8})\b', 'DRIVER_LICENSE', CONFIDENCE_BORDERLINE, 1, re.I)

# --- Georgia: 7-9 digits (with context) ---
add_pattern(r'(?:GA|Georgia|DL)[:\s]+(\d{7,9})\b', 'DRIVER_LICENSE', CONFIDENCE_BORDERLINE, 1, re.I)

# --- Alabama: 7 digits (with context) ---
add_pattern(r'(?:AL|Alabama|DL)[:\s]+(\d{7})\b', 'DRIVER_LICENSE', CONFIDENCE_BORDERLINE, 1, re.I)

# --- Missouri: Letter + 5-10 digits OR 9 digits with context ---
add_pattern(r'(?:MO|Missouri|DL)[:\s]+([A-Z]?\d{5,10})\b', 'DRIVER_LICENSE', CONFIDENCE_BORDERLINE, 1, re.I)

# --- Tennessee: 7-9 digits (with context) ---
add_pattern(r'(?:TN|Tennessee|DL)[:\s]+(\d{7,9})\b', 'DRIVER_LICENSE', CONFIDENCE_BORDERLINE, 1, re.I)

# --- South Carolina: 5-11 digits (with context) ---
add_pattern(r'(?:SC|South\s+Carolina|DL)[:\s]+(\d{5,11})\b', 'DRIVER_LICENSE', CONFIDENCE_BORDERLINE, 1, re.I)
...
add_pattern(r'(?:DL|DLN)[:\s#]+(\d{2}\s+\d{6})', 'DRIVER_LICENSE', CONFIDENCE_MEDIUM, 1, re.I)
...
add_pattern(r'(?:ORGAN\s*DONOR|VETERAN)\s+(\d{10,15})\s*$', 'UNIQUE_ID', CONFIDENCE_LOW, 1, re.I)
...
add_pattern(r'(?:last\s*4|last\s*four)[:\s]+(\d{4})\b', 'SSN_PARTIAL', CONFIDENCE_WEAK, 1, re.I)
...
add_pattern(r'(?:SSN|Social\s*Security)[:\s#]+(\d{3}[.\xb7]\d{2}[.\xb7]\d{4})', 'SSN', CONFIDENCE_LOW, 1, re.I)  # dots/middle dots
...
add_pattern(r'(?:Account)\s*(?:Number|No|#)?[:\s#]+([A-Z0-9][-A-Z0-9]{5,19})', 'ACCOUNT_NUMBER', CONFIDENCE_LOW, 1, re.I)
...
add_pattern(r'(?:Patient\s+)?(?:Acct)\s*(?:Number|No|#)?[:\s#]+([A-Z0-9-]{6,20})', 'ACCOUNT_NUMBER', CONFIDENCE_LOW, 1, re.I)
add_pattern(r'(?:Invoice|Billing|Statement)\s*(?:Number|No|#)?\s*[:#]\s*([A-Z0-9-]{6,20})', 'ACCOUNT_NUMBER', CONFIDENCE_WEAK, 1, re.I)
...
add_pattern(r'(?:Card|Credit\s*Card|CC|Payment)[:\s#]+(\d{4}[\s-]?\d{4}[\s-]?\d{4}[\s-]?\d{1,7})', 'CREDIT_CARD', CONFIDENCE_HIGH_MEDIUM, 1, re.I)
...
add_pattern(r'(?:VIN|Vehicle\s*(?:ID|Identification)(?:\s*Number)?)[:\s#]+([A-HJ-NPR-Z0-9]{17})\b', 'VIN', CONFIDENCE_NEAR_CERTAIN, 1, re.I)
...
# === License Plate ===
add_pattern(r'(?:License\s*Plate|Plate\s*(?:Number|No|#)|Tag)[:\s#]+([A-Z0-9]{2,8})', 'LICENSE_PLATE', CONFIDENCE_MEDIUM_LOW, 1, re.I)
...
# NDC with label
add_pattern(r'(?:NDC|National\s+Drug\s+Code)[:\s#]+(\d{5}-?\d{4}-?\d{2})', 'NDC', CONFIDENCE_HIGH, 1, re.I)
# 10-digit NDC without dashes (some formats)
add_pattern(r'(?:NDC)[:\s#]+(\d{10,11})\b', 'NDC', CONFIDENCE_MEDIUM_LOW, 1, re.I)

# === Room/Bed Numbers ===
# Hospital room numbers - require context
add_pattern(r'(?:Room|Rm\.?|Unit)[:\s#]+(\d{1,4}[A-Z]?)\b', 'ROOM_NUMBER', CONFIDENCE_MEDIUM_LOW, 1, re.I)
add_pattern(r'(?:Bed|Bay)[:\s#]+(\d{1,2}[A-Z]?)\b', 'BED_NUMBER', CONFIDENCE_MEDIUM_LOW, 1, re.I)
# Combined: "Room 412, Bed 3" or "Room 412-B"
add_pattern(r'(?:Room|Rm\.?)\s*(\d{1,4}[-]?[A-Z]?),?\s*(?:Bed|Bay)\s*(\d{1,2}[A-Z]?)', 'ROOM_NUMBER', CONFIDENCE_MEDIUM, 0, re.I)
# Floor + Room: "4th floor, room 412" or "Floor 4 Room 12"
add_pattern(r'(?:Floor|Fl\.?)\s*(\d{1,2})\s*[,\s]+(?:Room|Rm\.?)\s*(\d{1,4})', 'ROOM_NUMBER', CONFIDENCE_LOW, 0, re.I)

# === Pager Numbers ===
add_pattern(r'(?:Pager|Beeper|Pgr\.?)[:\s#]+(\d{3}[-.\s]?\d{3}[-.\s]?\d{4})', 'PAGER', CONFIDENCE_MEDIUM, 1, re.I)
add_pattern(r'(?:Pager|Pgr\.?)[:\s#]+(\d{4,7})\b', 'PAGER', CONFIDENCE_LOW, 1, re.I)  # Short pager codes

# === Extension Numbers ===
add_pattern(r'(?:ext\.?|extension|x)[:\s#]*(\d{3,6})\b', 'PHONE_EXT', CONFIDENCE_LOW, 1, re.I)
# Phone with extension: "555-1234 ext 567"
add_pattern(r'(\d{3}[-.\s]?\d{3}[-.\s]?\d{4})\s*(?:ext\.?|x)\s*(\d{3,6})', 'PHONE', CONFIDENCE_MEDIUM, 0, re.I)

# === Prior Authorization / Claim Numbers ===
add_pattern(r'(?:Prior\s*Auth(?:orization)?|PA)[:\s#]+([A-Z0-9]{6,20})', 'AUTH_NUMBER', CONFIDENCE_MEDIUM, 1, re.I)
add_pattern(r'(?:Auth(?:orization)?\s*(?:Number|No|#|Code))[:\s#]+([A-Z0-9]{6,20})', 'AUTH_NUMBER', CONFIDENCE_MEDIUM_LOW, 1, re.I)
add_pattern(r'(?:Pre-?cert(?:ification)?)[:\s#]+([A-Z0-9]{6,20})', 'AUTH_NUMBER', CONFIDENCE_MEDIUM_LOW, 1, re.I)
# Workers comp claim
...
add_pattern(r'(?:Type)[:\s]+([ABO]{1,2}[+-])(?:\s+blood|\s+Rh)', 'BLOOD_TYPE', CONFIDENCE_MEDIUM_LOW, 1, re.I)

# === Height (with context) ===
add_pattern(r'(?:Height|Ht\.?)[:\s]+(\d{1,2}[\'′]\s*\d{1,2}[\"″]?)', 'HEIGHT', CONFIDENCE_MEDIUM, 1, re.I)  # 5'10" format
add_pattern(r'(?:Height|Ht\.?)[:\s]+(\d{2,3})\s*(?:cm|in(?:ches)?)', 'HEIGHT', CONFIDENCE_MEDIUM_LOW, 1, re.I)  # metric/inches
add_pattern(r'(?:Height|Ht\.?)[:\s]+(\d\s*ft\.?\s*\d{1,2}\s*in\.?)', 'HEIGHT', CONFIDENCE_MEDIUM_LOW, 1, re.I)  # "5 ft 10 in"

# === Weight (with context) ===
add_pattern(r'(?:Weight|Wt\.?)[:\s]+(\d{2,3})\s*(?:lbs?|pounds?|kg|kilograms?)', 'WEIGHT', CONFIDENCE_MEDIUM_LOW, 1, re.I)
add_pattern(r'(?:Weight|Wt\.?)[:\s]+(\d{2,3}(?:\.\d)?)\s*(?:lbs?|kg)', 'WEIGHT', CONFIDENCE_MEDIUM_LOW, 1, re.I)
...
add_pattern(r'(?:NHS)[:\s#]+(\d{10})\b', 'NHS_NUMBER', CONFIDENCE_MEDIUM, 1, re.I)

# === Canadian SIN (9 digits, starts with specific digits) ===
add_pattern(r'(?:SIN|Social\s+Insurance)[:\s#]+(\d{3}[-\s]?\d{3}[-\s]?\d{3})', 'SIN', CONFIDENCE_RELIABLE, 1, re.I)
# Bare SIN with Canadian context (require word boundary for CA to avoid matching "Call")
add_pattern(r'(?:\bCanada\b|\bCanadian\b|\bCA\b)[^.]{0,30}(\d{3}[-\s]?\d{3}[-\s]?\d{3})', 'SIN', CONFIDENCE_WEAK, 1, re.I)

# === Australian TFN (Tax File Number - 8-9 digits) ===
add_pattern(r'(?:TFN|Tax\s+File)[:\s#]+(\d{3}\s?\d{3}\s?\d{2,3})', 'TFN', CONFIDENCE_RELIABLE, 1, re.I)

# === Indian Aadhaar (12 digits with specific format) ===
add_pattern(r'(?:Aadhaar|UIDAI|Aadhar)[:\s#]+(\d{4}\s?\d{4}\s?\d{4})', 'AADHAAR', CONFIDENCE_RELIABLE, 1, re.I)
add_pattern(r'(?:Aadhaar|UIDAI)[:\s#]+(\d{12})\b', 'AADHAAR', CONFIDENCE_MEDIUM, 1, re.I)

# === Mexican CURP (18 alphanumeric, specific format) ===
add_pattern(r'(?:CURP)[:\s#]+([A-Z]{4}\d{6}[HM][A-Z]{5}[A-Z0-9]\d)', 'CURP', CONFIDENCE_HIGH, 1, re.I)

# === German Sozialversicherungsnummer (12 digits) ===
add_pattern(r'(?:Sozialversicherungsnummer|SVNR|SV-Nummer)[:\s#]+(\d{2}\s?\d{6}\s?[A-Z]\s?\d{3})', 'SVNR', CONFIDENCE_RELIABLE, 1, re.I)

...
def from_presidio_results(
    ...
    """
    Convert Presidio analyzer results to normalized input.

    Args:
        results: List of RecognizerResult objects or dicts
        metadata: Optional file metadata dict

    Returns:
        NormalizedInput
    """
    adapter = PresidioAdapter()
    return adapter.extract(results, metadata)
...
def analyze_and_normalize(
    ...
    """
    Analyze text with Presidio and return normalized input.

    Requires presidio-analyzer to be installed.
    ...
    try:
        ...
        raise ImportError(
            "presidio-analyzer is required for this function. "
...
def analyze_file_and_normalize(
    file_path: str,
    language: str = "en",
    metadata: Optional[Dict[str, Any]] = None,
) -> NormalizedInput:
    """
    Analyze a file with Presidio and return normalized input.

    Args:
        file_path: Path to file
        language: Language code (default: "en")
        metadata: Optional additional metadata (path, size, etc. auto-detected)
    ...
    adapter = PresidioAdapter()
    return adapter.extract(results, auto_meta)
